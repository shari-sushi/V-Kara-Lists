...
func (interactor *VtuberContentInteractor) GetMovie(url string) (domain.Movie, error) {
...
func (interactor *VtuberContentInteractor) CreateKaraokes(inputterID domain.ListenerId, ks []domain.Karaoke) error {
	for i, k := range ks {
	...
	if err := interactor.VtuberContentRepository.CreateKaraokes(ks); err != nil {
		return err
	}
