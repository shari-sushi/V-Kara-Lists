...
// baseVideoSongQuery は vtubers×videos×video_songs×favorite_video_songs を
// LEFT JOIN した共通クエリ。呼び出し側で Where/Group/Order/Limit を追加する。
func (db *favoriteRepository) baseVideoSongQuery() *gorm.DB {
	...
	return db.Model(vt).Select(selectQu1, selectQu2, selectQu3, selectQu4).Joins(joinQu)
}
...
func (db *favoriteRepository) GetVtubersVideosVideoSongsWithFavCnts() ([]domain.TransmitVideoSong, error) {
	...
	groupQu := "vs.id"
	err := db.baseVideoSongQuery().Where(whereQu).Group(groupQu).Scan(&TmVss).Error
...
func (db *favoriteRepository) GetVtubersVideosVideoSongsByVtuberKanaWithFavCnts(kana string) ([]domain.TransmitVideoSong, error) {
	...
	groupQu := "vs.id"
	err := db.baseVideoSongQuery().Where(whereQu, kana).Group(groupQu).Scan(&TmVss).Error
...
func (db *favoriteRepository) GetLatest50VtubersVideosVideoSongsWithFavCnts(guestId domain.ListenerId) ([]domain.TransmitVideoSong, error) {
	...
	orderQu := "`vs`.`id` DESC"
	limitQu := 50
	err := db.baseVideoSongQuery().Where(whereQu1, guestId).Where(whereQu2).Group(groupQu).Order(orderQu).Limit(limitQu).Scan(&TmVss).Error
...
func (db *favoriteRepository) FindVideoSongsCreatedByListenerId(lId domain.ListenerId) ([]domain.TransmitVideoSong, error) {
	...
	groupQu := "vs.id"
	err := db.baseVideoSongQuery().Where(whereQu, lId).Group(groupQu).Scan(&TmVss).Error
	if err != nil {
		return TmVss, err
	}
