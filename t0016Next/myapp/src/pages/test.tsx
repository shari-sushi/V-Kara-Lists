...
export function CreateForm({posts,selectedVtuber, selectedMovie, selectedKaraoke}:selectedDate) {
    ...
    const onSubmit = async (CrudData:CrudDate) => {
        ...
        if (crudContentType === "vtuber"){
                createVtuber.VtuberId       = selectedVtuber;
                createVtuber.VtuberName     = CrudData.VtuberName;
                createVtuber.VtuberKana     = CrudData.VtuberKana;
                createVtuber.IntroMovieUrl  = CrudData.IntroMovieUrl;
                console.log("vtuberDataForFetch=", vtuberDataForFetch)
            }else if (crudContentType === "movie"){
                createMovie.VtuberId    = selectedVtuber;
                createMovie.MovieTitle  = CrudData.MovieTitle;
                createMovie.MovieUrl    = CrudData.MovieUrl;
                console.log("movieDataForFetch", movieDataForFetch)
            }else if (crudContentType === "karaoke"){
                createKaraoke.MovieUrl      = selectedMovie,
                createKaraoke.KaraokeListId = selectedKaraoke, //createでは0
                createKaraoke.SongName      = CrudData.SongName,
                createKaraoke.SingStart     = CrudData.SingStart,
        ...
        if(createVtuber || createMovie || createKaraoke){
            try {const response = await axios.post(`https://localhost:8080/create/${ crudContentType }`,
            createVtuber || createMovie || createKaraoke,
                    {
                    httpsAgent: process.env.NODE_ENV === "production" ? undefined : httpsAgent,
