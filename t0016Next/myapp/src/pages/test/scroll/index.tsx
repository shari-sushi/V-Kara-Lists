import { useGetMe, useGetUserWorkspaces } from "@/domains";
...
const DisplayJoinedWorkspaceLists = ({
  state,
}: {
  state: { summaries: WorkspaceSummary[]; user: { email: string } };
}) => {
  ...
  return (
    <>
      <div className="text-center text-[1.8rem] font-bold">
        &#x1f973;Welcome to PortalKey!
      </div>
      <div className="mt-12 p-0 rounded-md overflow-hidden">
        ...
        <div className="relative bg-surface-container-high">
          <div
            ref={scrollRef}
            className="max-h-[400px] m-0 list-none p-0 overflow-y-auto scrollbar"
          >
          ...
          {summaries.length > 4 && scrollPercentage > 10 && (
            <div
              className="absolute top-0.5 left-1/2 cursor-pointer opacity-40 hover:opacity-90 rounded-full overflow-hidden"
              onClick={scrollUp}
            >
              <ScrollUpIcon
                height={30}
                width={30}
                className="fill-gray-100 bg-gray-800 "
              />
            </div>
          )}
          {summaries.length > 4 && scrollPercentage < 99 && (
            <div
              className="absolute bottom-0.5 left-1/2 cursor-pointer opacity-40 hover:opacity-90 rounded-full overflow-hidden"
              onClick={scrollDown}
            >
              <ScrollDownIcon
                height={30}
...
const DummyIcon = ({}: {}) => {
  return (
    <div
      className={`flex text-sm rounded-sm w-full h-full bg-secondary-container text-on-primary overflow-hidden items-center justify-center no-underline`}
    >
  ...
  return (
    <div className="flex flex-col gap-2 justify-center ml-2">
      ...
      <div className="flex">
        ...
        <div className="ml-1">
          {summary.memberCount}人のメンバーが参加しています
        </div>
      </div>
    </div>
  );
