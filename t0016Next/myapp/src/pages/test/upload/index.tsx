      ...
      <div className="pt-4 px-1 w-[600px]">
        ...
        >
          <div className="flex h-10 justify-between items-center bg-gray-700 rounded-md overflow-hidden">
            <input className="w-[800px] h-full bg-transparent" id="file" onPaste={handlePasteFile} />
            <label htmlFor="file" className="flex w-10 justify-center items-center cursor-pointer hover:underline bg-transparent rounded-md ">
              <svg className={`${imagePreviews.length < 4 ? "fill-gray-200" : "fill-gray-600"}`} xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px">
              ...
              imagePreviews.map((image) => (
                <div key={`${image.previewID}`} className="">
                  <div className="relative">
                    {/* <img height={100} width={100} src={image.url} className="w-32 h-32 border-2 border-gray-600" /> */}
                    <div
              ...
              <div className="relative h-20 w-20 flex justify-center items-center">
                ...
                {hiddenButton !== true && (
                  <div className="absolute flex top-0 right-0 bg-on-secondary-container rounded-md shadow-md">
                    <DeleteButton onClick={() => deleteImage(image.previewID)} roundedClass="rounded-md" />
                  </div>
                )}
              </div>
            ) : (
              <div className={`relative  ${images.length === 1 ? "w-fit" : "h-32 w-full"}`}>
                <Image src={image.url} className={`w-full h-full rounded-md ${images.length === 1 ? "object-contain" : "object-cover"}`} alt="" />
                {hiddenButton !== true && (
...
export const DeleteButton = ({ onClick, height = 24, width = 24, roundedClass }: DeleteButtonProps) => {
  ...
  return (
    <div className={`${roundedClass}  h-fit w-fit justify-start items-center cursor-pointer bg-on-secondary-container hover:bg-on-surface`} onClick={handleClick}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" height={height} width={width} className="fill-darkred">
