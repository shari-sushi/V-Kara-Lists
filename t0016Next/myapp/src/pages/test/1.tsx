...
export default function App({ posts: data, isSignin }: CreatePageProps) {
  ...
  const handleclick = async (e: React.MouseEvent) => {
    ...
    try {
      const res: AxiosRequestConfig<BasicDataProps> = await axios.get(`${domain.backendHost}/vcontents/`)
      if (res.data) {
    ...
    return (
      <div className="p-1">
        <div onClick={handleclick}>
          <input type="button" value="click here!" className="px-1 bg-green-700 hover:underline cursor-pointer rounded-md" />
          {isClick ? <ShowApi vtubers={data.vtubers} vtubers_movies={data.vtubers_movies} vtubers_movies_karaokes={data.vtubers_movies_karaokes} /> : <p className="">表示されていません</p>}
        </div>

        <div onClick={() => setIsClick(false)}>
          <input className="bg-red-600 hover:underline cursor-pointer rounded-md" type="button" value="リセット"></input>
        </div>
      </div>
    )
  }
...
const ShowApi = ({ vtubers, vtubers_movies: movies, vtubers_movies_karaokes: karaokes }: BasicDataProps) => {
  return (
    <div>
