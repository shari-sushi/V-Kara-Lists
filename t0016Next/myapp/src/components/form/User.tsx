...
export const SigninForm = () => {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="bg-[#FFF6E4] shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full ">
        <div className="mb-2 tesxt-black">
          <div className="mb-2">
            ...
            <input className={`${ToClickTW.input}`} type="text" placeholder="v-karaoke@vkara.com" {...register("Email", ValidateEmail)} />
          ...
          <div className="mb-2">
            ...
            <input className={`${ToClickTW.input}`} placeholder="Password" type={isRevealPassword ? "text" : "password"} {...register("Password", ValidatePassword)} />
            <span className="text-red-400">
              {errors.Password && errors.Password.message} <br />
            </span>
            <Checkbox checked={isRevealPassword} onChange={() => setIsRevealPassword((state) => !state)}>
        ...
        <div className={`flex justify-between mx-6`}>
          <Link href="/" className={`${ToClickTW.formButton} flex w-[40%] justify-center `}>
            TOPへ戻る
          </Link>
          <Link href="/user/signup" className={`${ToClickTW.formButton} flex w-[40%] justify-center `}>
            会員登録へ
          </Link>
        </div>
      </form>
      {isOpenModal && <ErrorModal message={"ログインに失敗しました。"} setIsOpen={() => setIsOpenModal(false)} />}
    </>
  )
}
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="bg-[#FFF6E4] shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="mb-2">
          ...
          <input className={`${ToClickTW.input}`} id="username" type="text" placeholder="UserName" {...register("ListenerName", ValidateListenerName)} />
          <div className="text-red-400">{errors.ListenerName && errors.ListenerName.message}</div>
        </div>
        ...
        <div className={`flex justify-between mx-6`}>
          ...
          <Link href="/user/signin" className={`${ToClickTW.formButton} flex w-[40%] justify-center`}>
            ログインへ
          </Link>
        </div>
      </form>

      {isOpenModal && <ErrorModal message={"会員登録に失敗しました。"} setIsOpen={() => setIsOpenModal(false)} />}
    </>
  )
}

const ErrorModal = ({ message, setIsOpen }: { message: string; setIsOpen: () => void }) => {
  return (
    <>
      ...
      <button onClick={() => setIsOpen} className="absolute z-10 w-screen h-[145%] opacity-85 inset-0 bg-[#1f2724]"></button>
    </>
  )
}
