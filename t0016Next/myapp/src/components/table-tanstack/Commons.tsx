    ...
    cell: ({ row }: { row: { original: ReceivedKaraoke } }) => {
      ...
      return (
        <div className="relative flex w-auto">
          <button className={`flex ${LinkTW.base}`} onClick={() => handleClickPlay(row.original)}>
...
export const KaraokeTablePagenationButtons = ({ table, maxPageSize }: TablePagenationButtonsProps) => {
  return (
    <div className="flex bg-[#B7A692] mt-1 py-1 px-2 md:px-3 rounded-t-xl md:rounded-t-2xl max-w-[400px] ">
      ...
      <button className={`${TableCss.pageNationSingle} sm:mx-0.5 md:mx-1`} onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>
        {"<"}
      </button>
      <strong className="sm:mx-0.5 ">
        {table.getState().pagination.pageIndex + 1} / {table.getPageCount()}
      </strong>
      <button className={`${TableCss.pageNationSingle} sm:mx-1`} onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>
        {">"}
      </button>
      <button className={`${TableCss.pageNationDouble} md:mx-1`} onClick={() => table.setPageIndex(table.getPageCount() - 1)} disabled={!table.getCanNextPage()}>
        {">>"}
      </button>
